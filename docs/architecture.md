...
- **Repository Implementations**: The concrete implementation of the repository interface. It orchestrates data from one or more data sources. It depends on Data Source interfaces, not their concrete implementations.
...
- **Data Source Implementations**: The concrete implementation that contains the logic for fetching and writing data (e.g., using `fetch`, `@google/genai`, etc.).
...
4.  **Repository (`AI.repository.ts`)**: The repository implementation receives the call and delegates it to the `enhance()` method of the `IAIDataSource` interface it holds.
...
2.  **Implement Data Source**: Create the data source implementation in `src/features/new-feature/services/data-sources/`.
3.  **Implement Repository**: Create the repository implementation in `src/features/new-feature/services/`, ensuring it depends on the `INewFeatureDataSource` interface.
...
5.  **Wire Dependencies**: In `src/core/di/DIContainer.ts`, create and wire up your new data source, repository, and use case.
6.  **Create UI Hook**: Create a new React hook in `src/hooks/` to manage the feature's state and interaction with the use case.
7.  **Build the UI**: Create your React components, using the new hook to connect the UI to the application logic.
